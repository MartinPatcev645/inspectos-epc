...
if (!GOOGLE_API_KEY) {
  console.warn(
    "[AI] GOOGLE_API_KEY / GOOGLE_API_KEY_GAS is not set. /api/ai will return errors."
  );
}
...
function getSectionGuardConfig(sectionId: string) {
  ...
  > = {
    ...
    safety: {
      ...
      relevanceDefinition:
        "Assess CO detector presence and positioning compliance (height/proximity to appliance), detector condition and in-date status, ECV accessibility and clear labelling, adequacy of safety signage, and any visible unsafe conditions that justify immediate action (e.g., combustibles/naked flame near gas components).",
    },
  };
...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    ...
    console.error("[AI] Error in /api/ai handler:", err);
    res.status(500).json({
      error:
        err?.message ||
